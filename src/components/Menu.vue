...
<template>
  <aside
    class="absolute top-0 bottom-0 left-0 right-0 bg-white dark:bg-neutral-800 transition-all transform duration-500 will-change-transform"
    :class="{ closed }"
  >
    ...
    <RouterLink
      ...
      class="flex items-center justify-between mx-8 mb-4"
      to="profile"
      @click="close"
    >
    ...
    <div v-else class="flex items-center mx-8 mb-4">
      <button @click="login" class="px-4 py-2 bg-cyan-800 text-white rounded-lg w-full sm:w-auto">
        {{ $t('login') }}
      </button>
    </div>
    ...
    <div class="px-8 pt-0">
      ...
      >
        <div class="flex items-center">
          ...
          <p
            class="text-base text-black/80 dark:text-white font-light"
            :class="`pl-${link.paddingLeft}`"
          >
            {{ $t(link.path) }}
          </p>
        </div>
