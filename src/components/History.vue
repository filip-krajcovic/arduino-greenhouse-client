...
const lazyItems: Ref<IMeasurement[] | unknown[]> = ref(Array.from({ length: measurementsCount.value }))
...
const url = computed(() => `${API.baseUrl}/${API.messagesEndpoint}?skip=${skip.value}&limit=${limit.value}`)
...
watch(url, async endpoint => {
	if (cache.has(endpoint)) {
		...
		const data = result.map((value: IMeasurement) => ({...value, timestamp: new Date(value.timestamp)}))
...
<template>
	...
	</VirtualScroller></template>

<style lang="css">

.p-virtualscroller-content, .p-virtualscroller-loader {
	display: grid;
