...
<template>
	...
	<div class="grid grid-cols-2 gap-3 py-8">
		<div class="flex flex-col">
			...
			<InputMask class="w-24 text-lg dark:bg-black mt-2 px-4" v-model.lazy="timeOn" placeholder="00:00" mask="99:99" slotChar="hh:mm" />
		</div>
		<div class="flex flex-col">
			<label class="text-xl">{{ $t('turnOffTime') }}</label> 
			<InputMask class="w-24 text-lg dark:bg-black mt-2 px-4" v-model.lazy="timeOff" placeholder="00:00" mask="99:99" slotChar="hh:mm" />
		</div>
	</div>
	<div class="pt-4 flex">
		<div v-if="hourOn && minuteOn">
			{{ $t('lightsTurnOnFrom') }} <span class=" text-cyan-800 font-semibold mr-1"> {{ hourOn }}:{{ minuteOn }}</span>
		</div>
		<div v-if="hourOff && minuteOff">
			{{ $t('lightsTurnOffBy') }} <span class="text-cyan-800 font-semibold">{{ hourOff }}:{{ minuteOff }}</span>
		</div>
	</div>
	<div class="flex items-center mt-4">
      <button @click="saveData" class="px-4 py-2 bg-cyan-800 text-white rounded-lg w-full sm:w-auto">{{ $t('save') }}</button>
    </div>
</template>